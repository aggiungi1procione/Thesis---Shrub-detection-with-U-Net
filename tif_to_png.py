...
for file in glob.glob('PATH/images_tif/*.tif'):
    img = Image.open(file)                                                      
...
for file in glob.glob('PATH/images_png/*.png'):
    ...
    W_slider = int(tile_dim-((tile_dim-(img.width%tile_dim))/(img.width//tile_dim)))      
    ...
    for i in range(int((img.height-tile_dim)/H_slider)+1):
        for j in range(int((img.width-tile_dim)/W_slider)+1):
            ...
            img_crop.save('PATH/dataset_tiles/img'+ str(img_no) +'tile'+ str(tile_no) +'.png')
            x+=W_slider 
            j+=1
            tile_no+=1
        y+=H_slider
